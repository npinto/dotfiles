...
def setup_logging(output_dir=None):
    ...
    if output_dir:
        ...
        file_handler = logging.FileHandler(
            os.path.join(output_dir, "papermark_downloader.log")
        )
...
def bypass_email_protection(driver, email="temp@example.com"):
    ...
    try:
        ...
        continue_button = wait_for_element(
            driver, By.XPATH, "//button[contains(text(), 'Continue')]", timeout=3
        )
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                logger.info(
                    f"Email bypass successful after {time.time() - start_time:.2f} seconds"
                )
                return True
...
def get_current_page(driver):
    ...
    pagination_element = wait_for_element(
        driver,
        By.XPATH,
        "//div[contains(@class, 'flex h-8 items-center') or contains(@class, 'flex h-10 items-center')]",
        timeout=2,
    )
...
def wait_for_page_change(driver, current_page, timeout=5):
    ...
    while time.time() - start_time < timeout:
        ...
        if new_page is not None and new_page != current_page:
            logger.info(
                f"Page changed from {current_page} to {new_page} in {time.time() - start_time:.2f} seconds"
            )
...
def capture_all_slides(url, email=None, output_dir=None, max_failures=3):
    ...
    try:
        ...
        if not url.startswith("http://") and not url.startswith("https://"):
            logger.error(
                "ERROR: URL must begin with 'http://' or 'https://'. Aborting."
            )
            logger.error(f"Provided URL: {url}")
            logger.error(
                "Please provide a complete URL with http:// or https:// prefix"
            )
        ...
        while time.time() - start_time < max_wait:
            ...
            if current_page is not None:
                logger.info(
                    f"Page fully loaded in {time.time() - start_time:.2f} seconds"
                )
                break
            time.sleep(0.1)
        ...
        if requires_email(driver) and not email:
            logger.error(
                "Document requires an email; re-run with --email you@example.com"
            )
            driver.quit()
            sys.exit(1)
        ...
        while not at_last_page and consecutive_failures < max_failures:
            ...
            post_nav_page, post_nav_total = wait_for_page_change(
                driver, pre_nav_page, timeout=2
            )

            # If page didn't change, try a second time with more force
            if post_nav_page == pre_nav_page:
                logger.warning("First navigation attempt failed, trying again...")
                body.send_keys(Keys.RIGHT)
                post_nav_page, post_nav_total = wait_for_page_change(
                    driver, pre_nav_page, timeout=2
                )
            ...
            if total_pages and pre_nav_page == total_pages:
                logger.info(
                    f"Already at the last page ({pre_nav_page}/{total_pages}). Stopping capture."
                )
                at_last_page = True
                break
            ...
            if post_nav_page != pre_nav_page and post_nav_page == next_page:
                ...
                logger.info(
                    f"Successfully navigated to page {page_num} in {page_time:.2f} seconds"
                )

                # Check if we've reached the last page
                if total_pages and page_num == total_pages:
                    logger.info(
                        f"Reached last page ({page_num}/{total_pages}). Stopping capture."
                    )
                    at_last_page = True
                    break
            else:
                ...
                logger.warning(
                    f"Failed to navigate to next page. Current page: {post_nav_page}, Expected: {next_page}"
                )
                logger.warning(
                ...
                if total_pages and post_nav_page == total_pages:
                    logger.info(
                        f"Detected last page ({post_nav_page}/{total_pages}). Stopping capture."
                    )
                    at_last_page = True
                    break
        ...
        if consecutive_failures >= max_failures:
            logger.info(
                f"Reached {consecutive_failures} consecutive navigation failures. Stopping capture."
            )
        ...
        if page_times:
            ...
            logger.info(
                f"Time per page - Avg: {avg_time:.2f}s, Min: {min_time:.2f}s, Max: {max_time:.2f}s"
            )

        logger.info(
            f"Successfully captured {slide_count} slides in {total_time:.2f} seconds"
        )
...
def compress_pdf(input_pdf, output_pdf, compression_level=3):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Ghostscript not found. PDF compression requires ghostscript to be installed."
            )
            return False
        ...
        logger.info(
            f"PDF compression successful. Compression ratio: {compression_ratio:.2f}%"
        )
        logger.info(
            f"Original size: {input_size / 1024 / 1024:.2f} MB, Compressed size: {output_size / 1024 / 1024:.2f} MB"
        )
    ...
    try:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"Available Tesseract languages: {', '.join(installed_langs)}"
                )

                # Filter out unavailable languages
                available_languages = [
                    lang for lang in languages if lang in installed_langs
                ]
                if not available_languages:
                    logger.warning(
                        f"None of the requested languages {languages} are available in Tesseract."
                    )
                    available_languages = ["eng"]  # Default to English

                if available_languages != languages:
                    logger.warning(
                        f"Using only available languages: {available_languages}"
                    )
                    languages_str = "+".join(available_languages)
            else:
                logger.warning(
                    "Could not determine available Tesseract languages. Using English only."
                )
                languages_str = "eng"
        except Exception as e:
...
def process_slides_to_pdf(screenshots_dir, output_dir=None, languages=None):
    ...
    if not ocr_success:
        ...
        if len(languages) > 1 and "eng" in languages:
            logger.warning(
                "OCR failed with multiple languages. Trying with English only..."
            )
            ocr_success = add_ocr_to_pdf(slides_pdf, slides_ocr_pdf, ["eng"])
            if ocr_success:
                logger.info("OCR completed successfully with English only.")
            else:
    ...
    if ocr_success and os.path.exists(slides_ocr_pdf):
        ...
        if not compress_success:
            ...
            logger.info(
                f"Copied uncompressed OCR'd PDF to: {slides_ocr_compressed_pdf}"
            )

    total_time = time.time() - start_time
    logger.info(f"PDF generation completed in {total_time:.2f} seconds")
    logger.info("PDF files saved:")
...
def download_and_process_document(
    url, email=None, output_dir=None, languages=None, max_failures=3
):
    ...
    screenshots_dir, slide_count = capture_all_slides(
        url, email, output_dir, max_failures
    )
    ...
    pdf_success = process_slides_to_pdf(
        screenshots_dir, output_dir=screenshots_dir, languages=languages
    )
    ...
    logger.info(
        f"Captured {slide_count} slides and generated PDF files in: {screenshots_dir}"
    )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Download Papermark document and convert to multiple PDFs"
    )
    ...
    parser.add_argument(
        "--output-dir", "-o", help="Output directory for screenshots and PDFs"
    )
    parser.add_argument(
        "--languages",
        "-l",
