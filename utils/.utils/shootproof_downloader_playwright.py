#!/usr/bin/env python3
"""
ShootProof Robust Gallery Downloader - Playwright Version
...
class RobustShootProofDownloaderPlaywright:
    """Production-ready ShootProof gallery downloader using Playwright"""
    ...
    def __init__(self, config: DownloadConfig):
        ...
        # Log configuration
        self.logger.info("=" * 80)
        self.logger.info("ShootProof Robust Gallery Downloader - Playwright Version")
        self.logger.info("=" * 80)
    ...
    def _setup_logging(self):
        ...
        log_file = log_dir / f"shootproof_download_playwright_{timestamp}.log"
        ...
        # Setup logger
        self.logger = logging.getLogger("ShootProofPlaywright")
    ...
    def _playwright_context(self):
        """Context manager for Playwright lifecycle"""
        p = None
        browser = None
        page = None
        try:
    ...
    def _safe_wait_for_selector(
        ...
        """Safely wait for element with timeout"""
        try:
    ...
    def _safe_click(self, selector: str, force: bool = True) -> bool:
        """Safely click element"""
        try:
            self.page.click(selector, force=force, timeout=5000)
            return True
        except Exception as e:
    ...
    def authenticate(self):
        ...
        try:
            ...
            self.page.goto(self.config.gallery_url, wait_until="networkidle")
    ...
    def _auth_strategy_standard(self) -> bool:
        ...
        view_button_selectors = [
            ...
            "text=View Gallery",
            "//button[contains(text(), 'View Gallery')]"
        ]
        ...
        for selector in email_selectors:
            email_input = self._safe_wait_for_selector(selector, timeout=5000)
            if email_input:
                break
        ...
        self.page.fill(selector, self.config.email)
        time.sleep(0.5)
        ...
        for selector in selectors:
            try:
                ...
                if elem:
                    count_text = (
                        elem.inner_text() or
                        elem.get_attribute("content") or
                        elem.get_attribute("data-photo-count")
                    )
    ...
    def generate_report(self):
        ...
        # Save report
        report_file = Path(self.config.output_dir) / "download_report_playwright.txt"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ShootProof Robust Gallery Downloader - Playwright Version",
...
Examples:
  ...
  %(prog)s "https://gallery.url/" "email@example.com" --resume
    ...
    # Run downloader
    try:
        downloader = RobustShootProofDownloaderPlaywright(config)
        downloader.run()
        sys.exit(0)
    except KeyboardInterrupt:
        print("\n\nInterrupted by user")
